...
'''
    ...
    INIT : 
       ...
       1-  The "Options" parameter has the structure bellow , this parameter is the mandatory configuration of these class.
        ...
        Options = {
            ...
            "C" : SVM margin soft margin for small values and hard margin for higher values ---> Optional , Compulsory if GridCVSearch is not used <dtype: Int>
            "gamma" : Indicates the gamma used in SVR ---> Compulsory ("scale" or "auto" ) <dtype : string>
            "kernel" : Determines the kernel function in SVR (Kernel trick) --> Optional ("rbf" or "linear") <dtype : string>
    ...
    2- GridCVSearch params determines whether hypertunning optimization is performed : <dtype:bool>
    3- save_scaler_info : {"address": , "name": }
...
class svm(MLmodels):
    def __init__(self,options=None,loading=False):
        ...
        if options is not None and loading is False:
            ...
            if options.get("kernel") is not None:
    ...
    def fit(self,val_size,test_size,random_states,shuffle=False,custom_split = False ,split_args=None):
        ...
        self.data = data
        if self.GridCVSearch :
            grid = GridSearchCV(SVR(kernel=self.kernel, gamma=self.gamma), param_grid=self.params)
